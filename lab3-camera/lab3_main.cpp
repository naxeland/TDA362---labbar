...
void display()
{
	...
	mat4 m2 = glm::translate(vec3(20.0f, 0.0f, 0.0f)) * glm::rotate(float(M_PI * currentTime), worldUp) * translate(vec3(-10, 0, 0));


	modelViewProjectionMatrix = projectionMatrix * viewMatrix * m2;
	glUniformMatrix4fv(mvploc, 1, false, &modelViewProjectionMatrix[0].x);
	glUniformMatrix4fv(mloc, 1, false, &carModelMatrix[0].x);
	render(carModel);

	mat4 m3 = glm::translate(vec3(0.0f, 0.0f, 20.0f)) * glm::rotate(float(1 * currentTime), vec3(0,1,0)) * translate(vec3(-10, 0, 0));


	modelViewProjectionMatrix = projectionMatrix * viewMatrix * m3;
	glUniformMatrix4fv(mvploc, 1, false, &modelViewProjectionMatrix[0].x);
	glUniformMatrix4fv(mloc, 1, false, &carModelMatrix[0].x);
	render(carModel);
